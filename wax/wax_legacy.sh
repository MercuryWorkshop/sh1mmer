...
patch_bootloader() {
	...
	log_debug "$final_sector" "$sector_size" $((final_sector + 1)) $((BOOTLOADER_PART_SIZE / sector_size))
	"$CGPT" add "$LOOPDEV" -i 4 -b $((final_sector + 1)) -s $((BOOTLOADER_PART_SIZE / sector_size)) -t rootfs -l ROOT-A
	partx -u -n 4 "$LOOPDEV"
	suppress mkfs.ext2 -F -L ROOT-A "${LOOPDEV}p4"
...
patch_sh1mmer() {
	...
	partx -u -n 1 "$LOOPDEV"
	suppress mkfs.ext4 -F -L SH1MMER "${LOOPDEV}p1"
